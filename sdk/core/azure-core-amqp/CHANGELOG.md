...
- The uAMQP message sender now writes the detach condition and the description each time the service detaches its link. That line was behind the AMQP trace option, which is off by default, so a producer got no record of why the service ended the link. The message receiver already writes this line for every detach.
